...
def test_033d_n10b_invalid_runtime_module_is_error_then_done(client, monkeypatch):
    _enable_audit(monkeypatch)
    calls = {"stream": 0}
    ...
    assert response.status_code == 200
    assert _sse_event_names(response.text) == ["error", "done"]
    assert "AUDIT_RUNTIME_MODULE_NOT_ALLOWED" in response.text
    assert '"status": "failed"' in response.text
    assert calls["stream"] == 0


def test_033d_n10c_natan_readonly_denies_artifact_write_before_llm(client, monkeypatch):
    _enable_audit(monkeypatch)
    calls = {"stream": 0}
    ...
    response = client.post(
        f"/api/v2/threads/{thread_id}/stream",
        json={
            "content": "Crie um arquivo chamado teste_nata.txt no servidor e grave nele a palavra OK.",
            "agent": "Natã",
        },
        headers=headers(),
    )
    ...
    assert response.status_code == 200
    assert _sse_event_names(response.text) == ["error", "done"]
    assert "AUDIT_READONLY_WRITE_FORBIDDEN" in response.text
    assert '"governance_mode": "audit_readonly"' in response.text
    assert '"write_executed": false' in response.text
    assert calls["stream"] == 0
    with Testing() as db:
