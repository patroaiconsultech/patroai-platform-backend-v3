...
def test_033d_p02_sse_runtime_marker_proves_ledger_rehash_before_first_chunk(
    ...
    async def fake_stream(settings, agent, history):
        ...
        binding = payload["data"]["request_binding"]
        assert binding["contract"] == "ORKIO-AUDIT-REQUEST-BINDING-1"
        assert binding["operation"] == "runtime.search_marker"
        assert binding["arguments"]["module_id"]["value"] == "routes"
        assert binding["arguments"]["marker"]["binding"] == "sha256"
        assert (
            binding["arguments"]["marker"]["sha256"]
            == hashlib.sha256(b"send_message").hexdigest()
        )
        assert (
            payload["data"]["request_result_binding"]["marker_sha256_matches_request"]
            is True
        )
